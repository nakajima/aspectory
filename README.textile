...
Use the @#pristine@ method to call your methods without any callbacks.
Here's an example with the same example class we used above:

<pre>
something = Something.new
something.pristine(:foo)
something.results # => [:foo]
something.pristine(:bar)
